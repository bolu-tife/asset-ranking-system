...
public class MathUtils {
	...
	public static double std(List<Double> in) {
		...
		double squareResiduals = in.stream().map(e -> e - m).map(e -> e * e).reduce(0d, Double::sum);
	...
	public static List<Double> cumSum(List<Double> in) {
		...
		return in.stream().map(cumSum::calculate).collect(Collectors.toList());
	}
	...
	public static List<Double> ewm(List<Double> in, int span) {
		...
		return in.stream().map(exponentialMovingAverage::calculate).collect(Collectors.toList());
	}

	private static class TrueRange {
		private StockPrice prev;

		public double calculate(StockPrice period) {
			if (prev == null) {
				prev = period;
			}
	...
	public static List<Double> trueRange(List<StockPrice> in) {
		TrueRange trueRange = new TrueRange();

		return in.stream().map(trueRange::calculate).collect(Collectors.toList());
	}

	public static List<Double> diff(List<Double> in) {
		Diff diff = new Diff();

		return in.stream().map(diff::calculate).collect(Collectors.toList());
	}
	...
	public static double truePrice(StockPrice tradePeriod) {
	...
	public static <T> Stream<List<T>> expanding(List<T> list) {
		return IntStream.range(0, (list.size() - 1)).mapToObj(end -> list.subList(0, end));
	}
